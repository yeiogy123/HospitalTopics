...
class Unet_CT_SS(object):
    ...
    def loadTrainingData(self, images_path, labels_path, each):
        ...
        #a = 1
        #for i in images:
         #   fig = plt.imshow(i, cmap='gray', interpolation='nearest')
          #  plt.axis('off')
           # plt.savefig('image_' + str(a) +'.png', bbox_inches='tight',pad_inches = 0)
            #a += 1
        #a = 1
        #for i in labels:
            #fig = plt.imshow(i, cmap='gray', interpolation='nearest')
            #plt.axis('off')
            #plt.savefig('labels_' + str(a) + '.png', bbox_inches='tight', pad_inches=0)
            #a += 1
    ...
    def loadTestData(self, images_path, labels_path, each):
        ...
        if self.testLabelFlag:
            labels = nb.load(os.path.join(labels_path, each)).get_data()
            labels = labels.transpose(2, 0, 1).reshape(self.numImgs, self.img_rows, self.img_cols, 1).astype(
                self.dtypeL)
        else:
            labels = []
        return images, labels, affine
    ...
    def createModel(self):
        ...
        if self.optimizer == 'adam':
            optimizer = adam_v2.Adam(lr=lr, decay=decay)
            self.opt = optimizer
        elif self.optimizer == 'SGD':
            optimizer = gradient_descent_v2.SGD(lr=lr, decay=decay)
            self.opt = optimizer
    ...
    def createModel3D(self, size):
        if self.optimizer == 'adam':
            optimizer = adam_v2.Adam(lr=self.lr, decay=self.decay)
        elif self.optimizer == 'SGD':
            optimizer = gradient_descent_v2.SGD(lr=self.lr, decay=self.decay)
    ...
    def train(self):
        ...
        train_images_path, train_labels_path = self.arrangeDataPath(self.root_folder, self.image_folder,
                                                                    self.mask_folder)
        hist = {}
        hist['accuracy'] = []
        hist['loss'] = []
        for epochs in range(self.numEpochs):
            ...
            for each in os.listdir(train_images_path):
                ...
                history = model.fit(trainImages, trainLabels, batch_size=self.bs, epochs=1, verbose=1,
                                    sample_weight=wImg, validation_split=0.15)
            ...
            hist['accuracy'].append(acc / len(os.listdir(train_images_path)))
    ...
    def Predict(self, weights):
        ...
        for each in os.listdir(test_images_path):
            ...
            predImage = model.predict(testImages, batch_size=2, verbose=1)
            print('-' * 30)
            print('Predicting masks on test data...')
            print('-' * 30)
            # for comuting metrics of hyper dense class
            predImage = predImage.reshape((self.numImgs, self.img_rows, self.img_cols, self.nb_classes))[:, :, :,
                        self.sC - 1:self.sC]
    ...
    def Predict3D(self, weights):
        ...
        for each in os.listdir(test_images_path):
            ...
            if self.testLabelFlag:
                ...
                testLabels = testLabels.reshape((numImgs, img_rows, img_cols, img_dep, self.nb_classes))[0, :, :, :,
                             self.sC - 1:self.sC]
            ...
            print('-' * 30)
            # for comuting metrics of hyper dense class
            predImage = predImage.reshape((numImgs, img_rows, img_cols, img_dep, self.nb_classes))[:, :, :, :,
                        self.sC - 1:self.sC]
